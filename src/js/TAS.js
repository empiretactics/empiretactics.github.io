...
const TAStext = document.getElementById('TAStext');
...
// 在頁面加載時從 Cookie 載入內容
const elements = [Color, TACmyid, TACyourName, TACcd, TACtimes, TAStext, TACInsert, TACInsertName]; // 將所有需要讀取cookie的元素放入數組
window.onload = function() {
    ...
    // 若沒有值取預設值
    TAStext.value = TAStext.value || defaultText;
...
async function ResetAndRun() {
    ...
    let TwitchCommentTexts = TAStext.value.split('\n');
    TwitchCommentTexts = filterSpace(TwitchCommentTexts);
    TwitchCommentTexts = data0 + TACyourid + data1 + TwitchCommentTexts + data2;
